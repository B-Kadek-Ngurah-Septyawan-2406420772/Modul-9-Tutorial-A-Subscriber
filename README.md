...
AMQP stands for Advanced Message Queuing Protocol. It is an open standard protocol used by applications to communicate through a message broker. In this tutorial, AMQP is used so the subscriber can connect to RabbitMQ and receive events from the `user_created` queue. The subscriber does not need to know the publisher directly because it only listens to the queue managed by RabbitMQ. The publisher sends messages to the broker, and the broker stores or routes those messages until a subscriber consumes them. This makes the communication more loosely coupled because the publisher and subscriber can run as separate programs. With AMQP, the publisher and subscriber do not need to communicate directly because messages are sent through the broker first.

### What does `guest:guest@localhost:5672` mean?

In the URL `amqp://guest:guest@localhost:5672`, the first `guest` is the username used to authenticate to RabbitMQ. The second `guest` is the password for that username. The `localhost` part means the subscriber connects to a RabbitMQ server running on the same machine. The `5672` part is the port used for AMQP communication, and it is the default port used by RabbitMQ for application connections. This URL must match the broker used by the publisher so both applications communicate through the same RabbitMQ instance. If the subscriber uses a different host, port, username, or password, it may listen to the wrong broker or fail to connect. Therefore, this URL is the connection configuration that allows the subscriber to receive messages from the queue created in the tutorial.
